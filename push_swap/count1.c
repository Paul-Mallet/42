...
/*   Updated: 2025/02/08 18:50:53 by pamallet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	count_from_a_to_b(t_data *data)
{
	...
	while (++i < data->a.len)
	{
		...
		data->nb_ops = desc_rotate_nb_ops(data->a.arr[i], data, data->nb_ops, is_r); //TODO
		if (data->nb_ops < tmp) //1rst 12 = 3(rr), 2nd 2 = 3
		{
			tmp = data->nb_ops;
			tmp_i = i; // or i
		}
	...
	len_log_ops_from_a_to_b(data, tmp_i); //len_log_ops(rra, rrb)
}
